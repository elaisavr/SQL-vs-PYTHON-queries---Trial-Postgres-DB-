...
--- CREATE 2ND TABLE WITH RENTAL ORDERS DETAIL TO JOIN WITH CUSTOMERS FACT TABLE

CREATE TABLE order_details AS SELECT * FROM
    (SELECT * FROM payment
    LEFT JOIN inventory
    ON payment.rental_id = inventory.inventory_id) AS table1
LEFT JOIN 
    (SELECT store_id AS table2_store_id, manager_staff_id, 
            address_id FROM store) AS table2
ON table1.store_id = table2.table2_store_id;
